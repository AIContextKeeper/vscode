...
suite('Extension Test Suite', () => {
    ...
    test('Commands should be registered', async () => {
        ...
        assert.ok(
            commands.includes('contextkeeper.saveConversation'),
            'saveConversation command should be registered'
        );
        
        assert.ok(
            commands.includes('contextkeeper.showUsage'),
            'showUsage command should be registered'
        );
    });
    ...
    test('Configuration should have correct properties', () => {
        ...
        const defaultEndpoint = config.get<string>('apiEndpoint');
        assert.strictEqual(
            defaultEndpoint,
            'https://contextkeeper.dev/api',
            'Default API endpoint should be correct'
        );
    });
});